<script lang="ts">
	...
	import { getSeries, getStoriesBySeries, getCharactersBySeries } from '$lib/db';
	...
	import type { Series, Story, Character, StoryTheme } from '$lib/types';
	...
	async function refresh() {
		const id = seriesId;
		const [foundSeries, books, characters] = await Promise.all([
			getSeries(id),
	...
	// The book a series-shared character's profile link resolves through.
	...
	async function removeSeries() {
		...
		if (
			confirm(
				`Delete "${series.title}"? Its books become standalone stories, and shared characters move to ${orderedBooks[0]?.title ?? 'the first book'}.`
			)
		) {
